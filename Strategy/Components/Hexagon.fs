module Strategy.Components.Hexagon
...
type Hexagon =
    struct
        ...
        static member NewAxial q r =
            ...
            Hexagon(q, r, CalculateAxis q r)
            
        static member NewCube q r s =
            Hexagon(q, r, s)
            
        static member FromVector2 (pos : Vector2, hexfield_size) =
            let q = ((sqrt <| 3f) / 3f * pos.X - 1f / 3f * pos.Y) / hexfield_size
       ...
       member self.DistanceTo (other : Hexagon) =
           ...
           ((abs <| (self.Q - other.Q)) + (abs <| (self.R - other.R)) + (abs <| (self.S - other.S))) / 2
           
       member self.IsNeighbour other =
           self.DistanceTo other = 1
